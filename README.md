...
- 通过浏览器上传 **Excel 血糖数据文件** 和 **Word 模板文件**
- 从 Excel 中筛选各时间点（如 06:00、08:30、10:30 等）的血糖值
- 按照 Word 模板的格式，生成新的 Word 文档并支持下载
...
- 将记录按日期分组
- 对每个日期和每个时间点，在 `±window_minutes`（默认 30 分钟）窗口内查找时间距离最近的血糖读数
- 返回按日期和时间点组织的字典
...
2. 上传 **血糖数据 Excel 文件**（格式：`.xlsx` 或 `.xls`，包含时间列和血糖值列）
3. 上传 **Word 模板文件**（格式：`.docx`，第一行表头含时间点如 `06:00`）
4. 可选调整**时间匹配窗口**（分钟），默认 30 分钟
5. 点击"生成 Word 文档并下载"，浏览器自动下载生成的文档
